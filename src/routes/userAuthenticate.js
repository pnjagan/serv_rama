                ...
                bcrypt.hash(userData.password, saltRounds, function(err, hash) {
                    ...
                    if(!err){
                        ...
                        userModel.create(userDataDB).then(
                            s =>   { bouncer.reset(req); res.status(200).json({message: "user created successfully!"}); }
...
userAuth.post(
    ...
    , function (req, res) {
        ...
        //using weak equals to check for both undefined and null
        if ( userData.userLogin == undefined 
            || (    userData.password ==undefined 
                    && 
                    userData.jwtToken  ==undefined 
               )
        )  {
            log('login details invalid');
        ...
        if(userData.password !=undefined ) {
            userModel.getByAttribute('user_login',userData.userLogin).then(
                userRS => {
                    ...
                    bcrypt.compare(userData.password , userDataDB.userHash, function (errBcrypt, resBcrypt) {
                        if (!resBcrypt) {
                        ...
                        generateToken(userDataDB.id).then(
                            ...
                            r => {
                                ...
                                res.status(500).json( {message : 'Unexpected Login failure, please try again.'});
                            }
                        )
                                ...
                                , function (err, decoded) {
                                    ...
                                    (new User).getById(id)
                                        .then( rs => {
                                            ...
                                            log('CHECK*',rs.length,'*' ,rs[0].userLogin,'*',userData.userLogin,'*');
                                            log('check bool *',(rs.length ===1 && (rs[0].userLogin === userData.userLogin)) );
                                            ...
                                            if( 
                                                ...
                                                log('TOken error',rs.length,rs[0].userLogin,userData.userLogin);
